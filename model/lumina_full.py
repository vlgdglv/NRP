...
def load_lumina_full(model_path, checkpoint_path=None):
    ...
    if is_rank0():
        logger.info(f"Lumina full finetune: {trainable}/{total} params trainable ({100*trainable/total:.1f}%)")

    return model


class LuminaFullModel(nn.Module):
    def __init__(
        self,
        base_model,
        use_ce: bool = True,
